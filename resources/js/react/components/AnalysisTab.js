...
const AnalysisTab = ({analysis,analysisList,setAnalysisList}) => {   
      ...
      <>
         <Paper className="table-services" sx={{ width: '100%', overflow: 'hidden' }}>
            <FormControl fullWidth sx={{ m: 1, }} variant="standard">
               <TextField
                  onChange={ e => setAnalysisList(filterArrayByValue(analysis,e.target.value))}
                  label="Buscar análisis del laboratorio"
                  InputProps={{
                     startAdornment: (
            ...
            { analysisList <= 0 ? <Typography variant="h6" align="center" py={3}>No se encontraron resultados</Typography> : 
               <>
                  <TableContainer sx={{ maxHeight: 440 }}>
                     <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody>
                           {analysisList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(({name,price,locations}) => (                           
