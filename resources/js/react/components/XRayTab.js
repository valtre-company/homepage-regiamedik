...
const XRayTab = ({xRay,xRayList,setXRayList}) => {   
   if (xRay.length <= 0) return <Typography variant="h6" component="h6" align='center'>No disponible por el momento</Typography>         
   ...
   return ( 
      <>
         <Paper className="table-services" sx={{ width: '100%', overflow: 'hidden' }}>
            ...
            { xRayList <= 0 ? <Typography variant="h6" align="center" py={3}>No se encontraron resultados</Typography> : 
               <>
                  <TableContainer sx={{ maxHeight: 440 }}>
                     <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody>
                           {xRayList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(({name,price,locations}) => (                           
